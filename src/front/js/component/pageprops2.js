  ...
  useEffect(() => {
    ...
    const call_data = async () => {
      ...
      const final_array = data[cat].filter((file) => {
        return (
          (file.type_clothes === params.type_clothes &&
            file.gender === params.gender &&
            file.id !== 1) ||
          (file.type_prints === params.type_clothes &&
            file.gender === params.gender &&
            file.id !== 1)
        );
      });
      ...
      {loader ? (
        ...
        <div className="container-fluid padindg_category_list p_card_gender">
          <p className="text-muted">
            {params.category} / {params.gender} / {params.type_clothes}
          </p>
          <Link className="arrow_category_list" to={`/${params.category}`}>
              ...
              /*if(file.gender == "Men" && file.type_clothes == "Hoodies")*/ {
                return (
                  <Card_cat
                    ...
                    type={file.file_type}
                    type_clothes={
                      file.type_clothes ? file.type_clothes : file.type_prints
                    }
